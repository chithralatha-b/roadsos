  ...
  return (
    <div className="relative mx-auto min-h-screen w-full max-w-md overflow-hidden flex flex-col px-6 pt-10 pb-8">
      ...
      <div className="relative z-10 flex flex-col items-center mb-6">
        ...
        <h1 className="text-2xl font-bold">{step === "form" ? "Create your safety profile" : "Verify your phone"}</h1>
        <p className="text-xs text-muted-foreground mt-1">
          {step === "form" ? "Verified accounts only — required to use RoadSoS AI" : `OTP sent to +91 ${phone}`}
        </p>
      </div>
      ...
      {step === "form" ? (
        <div className="relative z-10 glass rounded-3xl p-5 space-y-4">
          ...
          <Field label="Insurance Provider">
            <input value={insurance} onChange={(e) => setInsurance(e.target.value)} placeholder="Star Health, HDFC Ergo…" className="flex-1 bg-transparent outline-none text-sm" />
          </Field>
          ...
          <button onClick={sendOtp} disabled={!ready} className="w-full py-3.5 rounded-2xl bg-gradient-ai glow-blue font-semibold text-white disabled:opacity-40 flex items-center justify-center gap-2">
        ...
        <div className="relative z-10 glass rounded-3xl p-6 space-y-5">
            ...
            {otp.map((d, idx) => (
              <input
                ...
                onChange={(e) => {
                  ...
                  if (v && idx < 3) (document.getElementById(`s-${idx + 1}`) as HTMLInputElement)?.focus();
                }}
                id={`s-${idx}`}
          ...
          <p className="text-[11px] text-muted-foreground text-center">Demo: enter any 4 digits to verify</p>
          <button onClick={verify} disabled={otp.join("").length !== 4} className="w-full py-3.5 rounded-2xl bg-gradient-ai glow-blue font-semibold text-white disabled:opacity-40">
            Verify & Continue
          </button>
          <button onClick={() => setStep("form")} className="w-full py-2 text-xs text-muted-foreground">Back to edit details</button>
        </div>
      )}
